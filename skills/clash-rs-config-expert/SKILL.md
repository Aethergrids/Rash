...
description: Create, review, migrate, validate, and troubleshoot Clash RS YAML configurations. Use when working with Clash RS config files, inbound listeners, TUN mode, DNS and fake-IP, outbound proxy protocols, proxy groups, routing rules, proxy or rule providers, external-controller settings, platform-specific configuration, Clash RS parser errors, or migration from Clash/Mihomo-compatible profiles.
---

# Clash RS Configuration Expert

Create the smallest secure configuration that satisfies the request. Treat the installed Clash RS binary as the final authority because configuration support changes by version and build features.
...
2. Load only the references needed for the task.
   - Read [core-and-tun.md](references/core-and-tun.md) for top-level settings, listeners, controllers, profiles, databases, and TUN.
   - Read [dns.md](references/dns.md) for DNS listeners, upstreams, fake-IP, policies, fallback, and TUN integration.
...
3. Preserve the user's configuration.
   ...
   - Explain behavior before changing routing, LAN exposure, TUN, DNS hijacking, or certificate verification.
...
- Enable `allow-lan`, TPROXY, TUN routing, or DNS hijacking only when the requested topology needs them.
